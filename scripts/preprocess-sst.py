...
class ConstTree(object):
    ...
    def get_labels(self, spans, labels, dictionary):
        ...
        if self.span not in dictionary:
            print(self.span)
        if self.left is not None:
...
def split(sst_dir, train_dir, dev_dir, test_dir):
    ...
    sents_train, sents_test, sents_dev = [], [], []
    with open(os.path.join(train_dir, 'sents.txt'), 'w') as train, \
        ...
        for sent, split, p in zip(sents, splits, parents):
            if split == 1:
                ...
                devparents.write(p)
                devparents.write('\n')
    return sents_train, sents_test, sents_dev
...
if __name__ == '__main__':
    ...
    sents_train, sents_test, sents_dev = split(sst_dir, train_dir, dev_dir, test_dir)
    ...
    write_labels(sents_train, train_dir, dictionary)
    write_labels(sents_dev, dev_dir, dictionary)
    write_labels(sents_test, test_dir, dictionary)
